...
/**
 * JPA-backed Repository implementation for OAuthToken entity.
 * Active when database.orm is 'jpa'.
 * Delegates all operations to the OAuthTokenDao interface (resolved to OAuthTokenDaoJpaImpl).
 */
@Repository
@RequiredArgsConstructor
@Slf4j
@ConditionalOnProperty(name = "database.orm", havingValue = "jpa")
public class OAuthTokenRepositoryJpaImpl implements OAuthTokenRepository {

    private final OAuthTokenDao tokenDao;

    @Override
    public OAuthToken save(OAuthToken token) {
        log.debug("JPA Repository: Saving OAuth token");
    ...
    public Optional<OAuthToken> findById(Long id) {
        log.debug("JPA Repository: Finding token by id: {}", id);
    ...
    public List<OAuthToken> findAll() {
        log.debug("JPA Repository: Finding all tokens");
        return tokenDao.findAll();
    }

    @Override
    public Page<OAuthToken> findAll(Pageable pageable) {
        log.debug("JPA Repository: Finding all tokens with pagination");
    ...
    public void deleteById(Long id) {
        log.info("JPA Repository: Deleting token by id: {}", id);
