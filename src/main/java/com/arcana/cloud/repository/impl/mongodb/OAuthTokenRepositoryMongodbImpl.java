...
/**
 * MongoDB-backed Repository implementation for OAuthToken entity.
 * Active when database.type is 'mongodb'.
 * Delegates all operations to the OAuthTokenDao interface (resolved to OAuthTokenDaoMongodbImpl).
 */
@Repository
@RequiredArgsConstructor
@Slf4j
@ConditionalOnProperty(name = "database.type", havingValue = "mongodb")
public class OAuthTokenRepositoryMongodbImpl implements OAuthTokenRepository {

    private final OAuthTokenDao tokenDao;

    @Override
    public OAuthToken save(OAuthToken token) {
        log.debug("MongoDB Repository: Saving OAuth token");
    ...
    public Optional<OAuthToken> findById(Long id) {
        log.debug("MongoDB Repository: Finding token by id: {}", id);
    ...
    public List<OAuthToken> findAll() {
        log.debug("MongoDB Repository: Finding all tokens");
        return tokenDao.findAll();
    }

    @Override
    public Page<OAuthToken> findAll(Pageable pageable) {
        log.debug("MongoDB Repository: Finding all tokens with pagination");
    ...
    public void deleteById(Long id) {
        log.info("MongoDB Repository: Deleting token by id: {}", id);
